...
export class UsuarioService extends BaseService {
    ...
    obterPorId(id: string): Observable<HttpApiResponse<DadosUsuario>> {
        return this.http
            .get<HttpApiResponse<DadosUsuario>>(this.UrlServiceCadastroUsuarioMonolito + "obterPorId/" + id, super.ObterAuthHeaderJsonUsuario())
            .pipe(catchError(super.serviceError));
    }
    ...
    atualizarUsuario(usuario: DadosUsuario): Observable<HttpApiResponse<any>> {
        ...
        let experiencia = usuario.experiencia;
        let cursos = usuario.cursos;
        return this.http.put<HttpApiResponse<any>>(`${this.UrlServiceCadastroUsuarioMonolito}atualizar/${id}`,{usuario,nome,email,data_nascimento,profissao,experiencia,cursos}, super.ObterAuthHeaderJsonUsuario());
    }
}